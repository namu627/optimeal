...
# Soft 제약(제공빈도·기호도·식단가) 모듈 — 같은 패키지. 스크립트/모듈 양쪽 실행 지원.
try:
...
class MealPlanRequest:
    ...
    # ── Soft 목적함수용 파라미터 (제공빈도·기호도·식단가 담당) ──────────────
    soft_weights: object = None                     # SoftWeights (None이면 기본값)
...
class MealPlanResult:
    ...
    soft_breakdown: dict = None   # Soft 항별 지표(제공빈도 충족·기호도·원가) — 미풀이 시 None
...
def build_and_solve(menus: list[MenuItem], req: MealPlanRequest) -> MealPlanResult:
    ...
    # 제공빈도·기호도·식단가 Soft 목적을 더한다. (색감·제철·중복 등 다른 Soft 항은
    # 각자 점수식을 만들어 아래 total_score 에 += 로 합산하면 됨 — 조합 가능 구조.)
    ...
    plan, daily_kcal, total_cost, breakdown = {}, {}, 0, None
    objective = 0.0
    if status in (cp_model.OPTIMAL, cp_model.FEASIBLE):
        ...
        breakdown = sc.evaluate_breakdown(
            solver, x, menus, soft,
            days=req.days, n_meals=len(req.meals),
            pref_scores=req.pref_scores,
        )
...
def print_result(res: MealPlanResult, req: MealPlanRequest):
    ...
    if res.soft_breakdown:
        ...
        print(f"  · 총 식재료비: {res.soft_breakdown['total_cost_won']:,}원")
