...
def pool(n_per_cat=12):
    ...
    픽스처 설계 주의 (둘 다 위반하면 제약이 아니라 픽스처 때문에 INFEASIBLE 이 된다):
      1. **열량 폭**을 실제 DB 수준으로 넓게 (주식 150~400 · 국 20~120 · 반찬류 20~250).
         좁으면 한 끼 최소 열량이 아침 밴드 상한(600×1.15=690)을 넘는다.
      2. **부찬은 끼니당 2개 이상**이므로 후보를 2배로 준다. 중복 금지 창이 걸리면
         창 안에서 필요한 서로 다른 부찬 수 = 2 × 끼니수 × 창일수 이다.

    Args:
        n_per_cat: 카테고리별 후보 수. 부찬만 이 값의 2배로 생성된다.
    """
